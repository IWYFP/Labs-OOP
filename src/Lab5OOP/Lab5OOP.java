...
public class Lab5OOP {
    static class VowelsCompare implements Comparator<Word> {
        public int compare(Word word1, Word word2) {
            ...
            Matcher matcher = pattern.matcher(word1.getWord());
            while (matcher.find()) number1++;
            matcher = pattern.matcher(word2.getWord());
            while (matcher.find()) number2++;
            return number1 - number2;
        }
    }
    public static void main(String[] args) {
        Text text = new Text("In literary theory, a text is any object that can be \"read\", whether" +
        ...
        for (Word word: words) System.out.print(word.getWord() + " ");
    }
}
